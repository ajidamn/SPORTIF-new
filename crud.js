...
const PAGES = {
    ...
    'orang': {
        ...
        columns: [
            ...
            { title:'Aksi', data:null, orderable:false, render:(d,t,r)=>actionBtns(r.id) }
        ],
    ...
    'organisasi': {
        ...
        fields:[
            ...
            {name:'telp',label:'Telepon',type:'text'},
    ...
    'sekolah': {
        ...
        columns: [
            ...
            { title:'Aksi', data:null, orderable:false, render:(d,t,r)=>`<a href="/admin/sekolah/${r.id}" class="btn btn-sm btn-info text-white me-1" title="Detail & Ekskul"><i class="bi bi-eye"></i></a>` + actionBtns(r.id) }
        ],
        fields:[
...
function buildFilterBar(filters) {
    ...
    filters.forEach(f => {
        ...
        if (f.type === 'select') {
            ...
            if (f.options) {
                f.options.forEach(o => opts += `<option value="${o.v}">${o.l}</option>`);
            } else if (f.cache) {
                const cacheData = window[f.cache] || eval(f.cache) || [];
                cacheData.forEach(o => opts += `<option value="${o[f.optionKey]}">${o[f.optionLabel]}</option>`);
            }
    ...
    for (const f of (cfg.fields||[])) {
        ...
        if (f.type === 'select') {
            ...
            list.forEach(o => {
                ...
                opts += `<option value="${v}" ${String(val)===String(v)?'selected':''}>${l}</option>`;
            });
            input = `<select class="form-select" name="${f.name}" id="f_${f.name}" ${f.required?'required':''}>${opts}</select>`;
...
function onEventInfoChange() {
    ...
    container.innerHTML = tab1HTML;
                ...
                <strong>${o.nama}</strong> <span class="text-muted small">· ${o.nik||'No NIK'} · ${o.domisili?.name||''}</span>
            </div>`).join('');
        resultsEl.style.display='block';
    } catch(e) {}
}

function selectOrang(idx, id, nama, nik) {
    document.getElementById(`pesertaOrangId_${idx}`).value = id;
    document.getElementById(`pesertaSearch_${idx}`).value = `${nama} (${nik})`;
    document.getElementById(`pesertaResults_${idx}`).style.display='none';
    document.getElementById(`pesertaSelected_${idx}`).innerHTML =
        `<small class="text-success"><i class="bi bi-check-circle me-1"></i>Orang ditemukan: ${nama}</small>`;
    populatePesertaCabor(idx);
}

function showEventPesertaForm() {
    document.getElementById('pesertaForm').style.display='block';
    if (editingId) populatePesertaCabor(0);
}

async function savePeserta(eventId) {
    const rows = document.querySelectorAll('[id^="pesertaRow_"]');
    const items = [];
    for (const row of rows) {
        const idx = row.id.split('_')[1];
        const orangId = document.getElementById(`pesertaOrangId_${idx}`)?.value;
        if (!orangId) { toast('error',`Baris ${parseInt(idx)+1}: Pilih orang yang valid dari autocomplete`); return; }
        items.push({
            orang_id:  orangId,
            cabor_id:  row.querySelector(`[name="pst_cabor_${idx}"]`)?.value || null,
            kategori:  row.querySelector(`[name="pst_kategori_${idx}"]`)?.value || null,
            prestasi:  row.querySelector(`[name="pst_prestasi_${idx}"]`)?.value || null,
            medali:    row.querySelector(`[name="pst_medali_${idx}"]`)?.value || null,
            tanggal:   row.querySelector(`[name="pst_tanggal_${idx}"]`)?.value || null,
            keterangan:row.querySelector(`[name="pst_keterangan_${idx}"]`)?.value || null,
        });
    }
    if (!items.length) { toast('error','Minimal 1 peserta'); return; }

    let success = 0;
    for (const item of items) {
        const r = await fetch(`/api/v1/orang/${item.orang_id}/riwayat`,{
            method:'POST', headers:{'X-CSRF-TOKEN':csrfToken,'Content-Type':'application/json','Accept':'application/json'},
            body:JSON.stringify({items:[{...item, event_id:eventId}]})
        });
        if (r.ok) success++;
    }
    toast('success',`${success} peserta berhasil disimpan`);
    await buildEventTabs({id:eventId});
}

async function deleteRiwayatFromEvent(riwayatId) {
    if (!confirm('Hapus data peserta ini?')) return;
    const r = await fetch(`/api/v1/riwayat-event/${riwayatId}`,{method:'DELETE',headers:{'X-CSRF-TOKEN':csrfToken,'Accept':'application/json'}});
    if (r.ok) { toast('success','Dihapus'); await buildEventTabs({id:editingId}); }
...
async function submitMultiTabForm() {
    ...
    if (pageSlug === 'orang') {
        ...
        const formData = new FormData(formEl);
        const data = {};
        ['nama','lokasi_id','alamat','pengelola','kapasitas','narahubung','telp_narahubung','keterangan','latitude','longitude']
        ...
        const formData = new FormData(formEl);
        const data = {};
        ['nama','jenis_id','jenis_event','skala_id','penyelenggara','lokasi_kegiatan','tanggal_mulai','tanggal_selesai','status','disabilitas']
